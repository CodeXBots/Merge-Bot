...
async def MergeVideo(input_file: str, user_id: int, message: Message, format_: str):
    ...
    """
    output_vid = f"downloads/{str(user_id)}/[@RahulReviewsYT].{format_.lower()}"
...
async def MergeSub(filePath: str, subPath: str, user_id):
    ...
    subTrack += 1
    subTitle = f"Track {subTrack} - tg@RahulReviewsYT"
    ...
    muxcmd.append("-c:s")
    muxcmd.append("srt")
    muxcmd.append(f"./downloads/{str(user_id)}/[@RahulReviewsYT]_softmuxed_video.mkv")
    LOGGER.info("Muxing subtitles")
    subprocess.call(muxcmd)
    orgFilePath = shutil.move(
        f"downloads/{str(user_id)}/[@RahulReviewsYT]_softmuxed_video.mkv", filePath
    )
    return orgFilePath
...
def MergeSubNew(filePath: str, subPath: str, user_id, file_list):
    ...
    for j in range(1, (len(file_list))):
        ...
        muxcmd.append(f"{j}:s")
        muxcmd.append(f"-metadata:s:s:{subTrack}")
        muxcmd.append(f"title=Track {subTrack+1} - tg@RahulReviewsYT")
        subTrack += 1
    muxcmd.append("-c:v")
    muxcmd.append("copy")
    muxcmd.append("-c:a")
    ...
    subprocess.call(muxcmd)
    return f"downloads/{str(user_id)}/[@RahulReviewsYT]_softmuxed_video.mkv"
...
def MergeAudio(videoPath: str, files_list: list, user_id):
    ...
    for j in range(1, len(files_list)):
        ...
        muxcmd.append(f"title=Track {audioTracks+1} - tg@RahulReviewsYT")
        audioTracks += 1
    ...
    muxcmd.append("-c:s")
    muxcmd.append("copy")
    muxcmd.append(f"downloads/{str(user_id)}/[@RahulReviewsYT]_export.mkv")

    LOGGER.info(muxcmd)
    process = subprocess.call(muxcmd)
    LOGGER.info(process)
    return f"downloads/{str(user_id)}/[@RahulReviewsYT]_export.mkv"
